...
def buscar_indice_por_titulo(catalogo: list[dict], titulo_busqueda: str) -> int:
    ...
    for i, libro in enumerate(catalogo):
        titulo_actual_norm = normalizar_titulo(libro["TITULO"])
...
def ingresar_titulos_multiples(catalogo: list[dict]) -> list[dict]:
    ...
    libros_agregados=0
    n = pedir_entero_no_negativo("¿Cuántos libros desea cargar? ")
    for i in range (n):
        print(f'\n titulo: {i}')
        titulo=input('Ingresa el titulo')
        if existe_titulo(catalogo, titulo):
            print('El titulo ya existe, este titulo se omitirá')
            continue
        cantidad_titulo=pedir_entero_no_negativo(f'cantidad inicial para {titulo}')
        catalogo.append({'TITULO': titulo, 'CANTIDAD': cantidad_titulo})
        libros_agregados+=1
        print('titulo pre-cargado')
    if libros_agregados>0:
        guardar_catalogo_a_csv(catalogo)
    else: 
        print('No se cargaron libros nuevos')
...
def ingresar_ejemplares(catalogo: list[dict]) -> list[dict]:
    ...
    Debe devolver el catálogo actualizado.
        implementar.
    """
    pedir_titulo_cambiar=input('ingrsa el titulo que desea sumar: ')
    mas_ejemplares=pedir_entero_no_negativo(f'ingresa los ejemplares para {pedir_titulo_cambiar}')
    if existe_titulo(catalogo, pedir_titulo_cambiar):
        for fila in catalogo:
            if fila['TITULO']==pedir_titulo_cambiar:
                fila['CANTIDAD']+=mas_ejemplares
                guardar_catalogo_a_csv(catalogo)
    else:
...
def consultar_disponibilidad(catalogo: list[dict]) -> None:
    ...
    no_encontrado=0
    num=0
    consultar_titulos=input('ingrese el titulo que desea condultar: ')
    for fila in catalogo:
        num+=1
        if fila['TITULO']==consultar_titulos:
            print(f'El titulo "{consultar_titulos}" Tiene {fila['CANTIDAD']} Ejemplares')
        else:
            no_encontrado+=1
    if no_encontrado==num:
        print(f'El titulo "{consultar_titulos}" no fue encontrado')
...
def listar_agotados(catalogo: list[dict]) -> None:
    ...
    for fila in catalogo:
        if fila['CANTIDAD']==0:
            print(fila)
            cant+=1
    print(f'cantidad de agotados: {cant}')
...
def agregar_titulo(catalogo: list[dict]) -> list[dict]:
    ...
    TODO: implementar.
    """
    titulo_agregar=input('ingresa el titulo a agregar: ')
    if titulo_valido(titulo_agregar):
        cant_titulo_agregar=int(input('ingrese los ejemplares: '))
        if cant_titulo_agregar>=0:
            nueva_linea={'TITULO':titulo_agregar, 'CANTIDAD':cant_titulo_agregar}
